...
for tag, kw in [("auto", {}), ("off", {"early_stopping": False}), ("on", {"early_stopping": True})]:
    t0=time.time()
    P = H(max_iter=200, learning_rate=0.1, max_bins=255, random_state=SEED, **kw).fit(X(pool), y(pool))
    pit = int(P.n_iter_)
    for s in subs:
        m = H(max_iter=200, learning_rate=0.1, max_bins=255, random_state=SEED, **kw).fit(X(tr[s]), y(tr[s]))
        yt = y(te[s])
